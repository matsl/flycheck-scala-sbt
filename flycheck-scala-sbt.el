...
(defun flycheck-scala-sbt--errors-list ()
  ...
  (save-excursion
    (let ((scala-errors '())
      ...
      (goto-char (point-min))
      (let ((old-errors (cl-make-hash-table :test #'equal)))
        (dolist (old-error (append sbt-errors scala-errors))
          (setf (cl-gethash (cons (first old-error) (second old-error)) old-errors) t))
        (while (re-search-forward flycheck-scala-sbt--java-regex (point-max) t)
          (let ((new-error (flycheck-scala-sbt--extract-weird-error-info flycheck-scala-sbt--java-regex)))
            (unless (cl-gethash (cons (first new-error) (second new-error)) old-errors)
